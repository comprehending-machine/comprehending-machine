...
def options_and_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Comprehending-Machine thinks for you about the meanings.",
        epilog="Example:  comprehending-machine input_text.txt > output_text.txt"
...
def run():
    """
    $ text | comprehending-machine                        # Accepts text from the pipe
    $ echo "...<text>..." | comprehending-machine         #

    $ comprehending-machine multilogue.txt new_turn.txt    # ...or files.
    """
    ...
    try:
        ...
        syslog.openlog(
            ident="comprehending-machine",
            logoption=syslog.LOG_NDELAY,
            facility=syslog.LOG_USER
        )
        # Signal (single line less than 4096 only!)
        syslog.syslog(syslog.LOG_INFO, f"comprehending-machine: {utterance}.")
        syslog.closelog()
